...
export function registerInternalRoutes(fastify, { staffQuery, runReadonly }) {
  ...
  const OPS = {
    ...
    snapshot_insert: [(p) => [JSON.stringify(p.insights), p.provider, p.commentary ?? null],
      `INSERT INTO insights_snapshots (tenant_id, insights, provider, commentary) VALUES (app.current_tenant(), $1::jsonb, $2, $3)`],
    insights_latest: [() => [], `SELECT insights, provider, created_at, commentary FROM insights_snapshots ORDER BY created_at DESC LIMIT 1`],
