...
export async function buildApp({ pool, customerPool, adminPool, aiServiceUrl, auth, logger = true } = {}) {
  ...
  // raw:true keeps numbers numeric and real Excel date cells as Date objects
  // (raw:false + dateNF formats them with the cell's own numFmt, e.g. "8/1/26",
  // which would then fail to cast into Postgres DATE). Normalize dates to ISO.
  const rows = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { raw: true, cellDates: true })
  ...
  const placeholders = rows.map((row) => {
    ...
    values.push(req.tenantId, ...fields.map((f) => {
      const v = row[f]
