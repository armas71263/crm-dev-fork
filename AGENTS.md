...
- **Supabase**: use the SESSION pooler (transaction pooler breaks session-level `set_config`; direct conn is IPv6-only). Pool `max: 5`. Automatic RLS on (safety net; our migrations ENABLE+FORCE explicitly anyway). Custom-role pooler usernames are `role.PROJECT_REF`.
