...
# DSN resolution: SUPABASE_DB_SESSION_POOLER_URL first, then PG_* env, then the
# docker-compose defaults. Runs as the admin (superuser) role on purpose:
# migrations create roles and policies; app traffic stays on app_role.
set -euo pipefail

DSN="${SUPABASE_DB_SESSION_POOLER_URL:-postgresql://${PG_ADMIN_USER:-postgres}:${PG_ADMIN_PASSWORD:-postgres}@${PG_HOST:-localhost}:${PG_PORT:-5432}/${PG_DATABASE:-rubbertrack}}"
