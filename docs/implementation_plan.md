...
| Predictions | **Combo A**: AutoGluon (Apache-2.0) — TimeSeries with Chronos-Bolt (Apache-2.0 weights) + AutoARIMA fallback; Tabular with Mitra challenger (Apache-2.0) behind the Logit default. TabFM excluded: pretrained weights are non-commercial (`tabfm-non-commercial-v1.0`). TimesFM optional later |
